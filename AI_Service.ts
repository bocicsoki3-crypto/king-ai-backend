// FÁJL: AI_Service.ts
// VERZIÓ: v112.1 (TS Export Fix)
// MÓDOSÍTÁS (v112.1):
// 1. JAVÍTÁS: Minden fő függvény (runStep_FinalAnalysis, getChatResponse, stb.)
//    elé kikerült az 'export' kulcsszó.
// 2. JAVÍTÁS: Törölve a fájl végi hibás 'export default' blokk, ami a TS18004-et okozta.
// 3. EREDMÉNY: A függvények most már 'named export'-ként érhetők el (pl. import { getChatResponse } ...).
...
export async function getAndParse(
    ...
    try {
        ...
        console.error(`[AI_Service v112.1] AI Hiba: A válasz JSON (${keyToExtract}) nem tartalmazta a várt kulcsot a ${stepName} lépésnél.`);
        return `AI Hiba: A válasz JSON nem tartalmazta a '${keyToExtract}' kulcsot.`;
    } catch (e: any) {
        console.error(`[AI_Service v112.1] Végleges AI Hiba (${stepName}): ${e.message}`);
        return `AI Hiba (${keyToExtract}): ${e.message}`;
    }
}

// === ÚJ: 0. ÜGYNÖK (DEEP SCOUT / SZUPER FELDERÍTŐ) PROMPT v2 ===
const PROMPT_DEEP_SCOUT_V2 = `
...
You must find "Hidden Information" and CRITICAL STATISTICS that standard APIs miss.

[INVESTIGATION DIMENSIONS]:
1. **XG HUNT (CRITICAL):**
   - Search for "Expected Goals (xG) stats {home} current season" and "{away} xG stats".
   - Find the **xG (For)** and **xGA (Against)** per 90 minutes (or total average) for BOTH teams.
   - If exact xG is not found, estimate based on "Goals Scored/Conceded per game".

2. **PHYSICAL STATE:**
   - Travel issues? (e.g. Did the away team travel 4000km?)
   - Fatigue? (Did they play <72 hours ago?)
   - Illness/Virus in the camp?

3. **PSYCHOLOGICAL STATE:**
   - Locker room morale? (Manager under fire? Player revolt?)
   - Motivation? (Must-Win for title/relegation vs Dead Rubber?)
   - Revenge narrative?

4. **TACTICAL INTEL:**
   - Leaked lineups? Key player benched?
   - Style clash news?

5. **EXTERNAL FACTORS:**
   - Weather impact? (Heavy snow, extreme heat, waterlogged pitch?)
   - Fan impact? (Empty stadium?)
...
Your response MUST be ONLY a single, valid JSON object with this EXACT structure:
{
  ...
  "xg_stats": {
      ...
      "source": "<String, e.g. 'FBref/Understat via Search' or 'Estimated'>"
  },
  "stats_fallback": {
...
Synthesize ALL available reports: Risk Assessment, Tactical Briefing, Prophetic Scenario, Statistical Simulation results, Micromodel conclusions, and overall Context.
Discuss the most promising betting angles considering both potential value and risk.

**STRATEGY PILLARS (v109.0):**
1. **MARKET WISDOM:** Does the context mention odds movement? If Opening > Current, "Smart Money" is confirming the bet. Mention this.
2. **MOTIVATION MATRIX:** Is it a "Must-Win", "Cup Final" or "Relegation Battle"? If yes, overweight the offensive potential.
3. **TACTICAL CLASH:** Look for "Style vs Style". (e.g., High Press vs Low Block). Does one style hard-counter the other?
...
Select the 'least bad' or most logical option even with uncertainty.**
...
4. Narrative Context: "{expertConfidence}" (Contains Motivation & Market Intel)
5. Risk Assessment: "{riskAssessment}"
6. Strategic Thoughts: "{strategicClosingThoughts}"

--- THE "PERFECT TIP" PROTOCOL (v109.0) ---

**STEP 1: MATH GUARDRAIL (THE FOUNDATION)**
- If statistical xG/Simulations are overwhelmingly strong (e.g. xG Total > Line + 0.4), DO NOT BET AGAINST IT based on "hunch". Stats are the base truth.

**STEP 2: MARKET WISDOM (THE VALIDATOR)**
- Scan the 'Context' for odds movement keywords (dropping/csökkenő, drifting/emelkedő).
- **Smart Money Rule:** If Opening Odds > Current Odds (price dropping), the market agrees with this outcome. INCREASE CONFIDENCE.
- **Trap Rule:** If public logic says Over, but odds are drifting up, be cautious.

**STEP 3: MOTIVATION MATRIX (THE X-FACTOR)**
- Is it a "Must-Win"? (Cup, Relegation, Title). If YES, ignore "poor form". Desperation = Goals/Aggression.
- Is it a "Dead Rubber"? If YES, expect randomness/Under.

**STEP 4: TACTICAL CLASH (THE EXECUTION)**
- Check 'Strategic Thoughts'.
- "High Press vs Counter" -> OVER / BTTS.
- "Possession vs Low Block" -> UNDER / Corners.

**DECISION ALGORITHM:**
- **BEST CASE:** Math + Market + Motivation ALL align. -> **MAX CONFIDENCE.**
- **GOOD CASE:** Math + Motivation align. -> **HIGH CONFIDENCE.**
- **VALUE CASE:** Math indicates value, Market disagrees but Logic supports Math. -> **VALUE BET.**
- **BAD CASE:** Math says X, Motivation says Y. -> **SKIP / LOW CONFIDENCE.**

OUTPUT FORMAT: Your response MUST be ONLY a single, valid JSON object with this EXACT structure:
{"recommended_bet": "<The SINGLE most compelling market (e.g., Hazai győzelem, Over 2.5, Warriors Over 115.5)>", "final_confidence": <Number between 1.0-10.0>, "brief_reasoning": "<SINGLE concise Hungarian sentence explaining the choice based on the 3 Pillars>"}
`;


// --- ÜGYNÖK FUTTATÓ FÜGGVÉNYEK ---

// === 0. ÜGYNÖK (DEEP SCOUT - v112.0) ===
// Export kulcsszó hozzáadva
export async function runStep_DeepScout(data: { home: string, away: string, sport: string }): Promise<any> {
    try {
        console.log(`[AI_Service v112.1] 0. ÜGYNÖK (DEEP SCOUT) INDÍTÁSA: ${data.home} vs ${data.away}...`);
        const filledPrompt = fillPromptTemplate(PROMPT_DEEP_SCOUT_V2, data);
        // ITT HASZNÁLJUK A 'useSearch: true' PARAMÉTERT!
        const result = await _callGeminiWithJsonRetry(filledPrompt, "Step_DeepScout", 2, true);
        
        console.log(`[AI_Service v112.1] Deep Scout Jelentés: "${result?.narrative_summary?.substring(0, 50)}..."`);
        if (result?.xg_stats) {
            console.log(`[AI_Service v112.1] xG ADAT TALÁLAT! H_xG: ${result.xg_stats.home_xg}, H_xGA: ${result.xg_stats.home_xga}`);
        }
        return result;
    } catch (e: any) {
        console.error(`[AI_Service v112.1] Deep Scout Hiba: ${e.message}`);
        return null;
    }
}
...
export async function runStep_TeamNameResolver(data: TeamNameResolverInput): Promise<number | null> {
    try {
        ...
        if (result && result.matched_id) {
            ...
            console.log(`[AI_Service v112.1 - Térképész] SIKER: Az AI a "${data.searchTerm}" nevet ehhez a csapathoz rendelte: "${matchedTeam?.name || 'N/A'}" (ID: ${foundId})`);
            return foundId;
        } else {
            console.error(`[AI_Service v112.1 - Térképész] HIBA: Az AI nem talált egyezést (matched_id: null) a "${data.searchTerm}" névre.`);
            return null;
        }
    } catch (e: any) {
        console.error(`[AI_Service v112.1 - Térképész] KRITIKUS HIBA a Gemini hívás vagy JSON parse során: ${e.message}`);
        return null;
    }
}
...
export async function runStep_Psychologist(data: PsychologistInput): Promise<any> {
    try {
        ...
        console.error(`[AI_Service v112.1] AI Hiba (Psychologist): ${e.message}`);
        return {
...
export async function runStep_Specialist(data: SpecialistInput): Promise<any> {
    try {
        ...
        console.error(`[AI_Service v112.1] AI Hiba (Specialist): ${e.message}`);
        return {
...
// === STRATÉGIA ÉS FŐNÖK (MÓDOSÍTVA v109.0) ===
    ...
    try {
        ...
        try {
            ...
            if (match && match[1]) {
                ...
                console.log(`[AI_Service v112.1 - Főnök] Expert bizalom sikeresen kinyerve: ${expertConfScore}`);
            } else {
                console.warn(`[AI_Service v112.1 - Főnök] Nem sikerült kinyerni az expert bizalmat: "${expertConfidence}". Alapértelmezett: 1.0`);
                expertConfScore = 1.0;
            }
        } catch(e: any) { 
            console.warn("[AI_Service v112.1 - Főnök] Hiba az expert bizalom kinyerésekor:", e);
            expertConfScore = 1.0;
        }
        ...
        // === 1. LÉPÉS: AI (Tanácsadó) hívása az APEX LOGIC Prompttal ===
        ...
        if (!rec || !rec.recommended_bet || typeof rec.final_confidence !== 'number') {
            console.error("[AI_Service v112.1 - Főnök] Master AI hiba: Érvénytelen JSON struktúra a válaszban:", rec);
            throw new Error("AI hiba: Érvénytelen JSON struktúra a MasterRecommendation-ben.");
        }
        
        // --- 2. LÉPÉS: KÓD (A "Főnök") átveszi az irányítást ---
        console.log(`[AI_Service v112.1 - Főnök] AI (Tanácsadó) javaslata: ${rec.recommended_bet} @ ${rec.final_confidence}/10`);

        // 1. Eltérés-alapú büntetés (Megengedőbb, mint a v107.0)
        const confidenceDiff = Math.abs(safeConfidenceOverall - expertConfScore);
        const disagreementThreshold = 4.0; 
        ...
        if (expertConfScore < 1.1 && expertConfidence && !expertConfidence.toLowerCase().includes("hiba")) {
            ...
            confidencePenalty = Math.min(1.5, confidenceDiff / 2); 
            disagreementNote = ` (FŐNÖK KORREKCIÓ: Enyhe korrekció az eltérő modellek miatt.)`;
        }
        ...
        try {
            ...
            if (calibrationMap && Object.keys(calibrationMap).length > 0) {
                ...
                if (calibrationMap[bucketKey] && calibrationMap[bucketKey].total >= 5) {
                    ...
                    if (Math.abs(calibratedConfidence - rec.final_confidence) > 1.0) { // Csak nagy eltérésnél szól
                        calibrationNote = ` (Kalibrált: ${calibratedConfidence.toFixed(1)}/10, ${total} minta.)`;
                    }
                }
            }
        } catch(calError: any) { 
            console.warn(`[AI_Service v112.1 - Főnök] Bizalmi kalibráció hiba: ${calError.message}`); 
        }
        ...
        console.log(`[AI_Service v112.1 - Főnök] VÉGLEGES Tipp: ${rec.recommended_bet} @ ${rec.final_confidence.toFixed(1)}/10`);
        
        return rec;

    } catch (e: any) {
        console.error(`[AI_Service v112.1 - Főnök] Végleges hiba a Mester Ajánlás generálása során: ${e.message}`, e.stack);
...
If the answer isn't in the context or history, politely state that the information is not available in the analysis.`;
        ...
        console.error(`[AI_Service v112.1] Chat hiba: ${e.message}`, e.stack);
