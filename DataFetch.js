...
* JAVÍTÁS (2025-10-26 v5):
* - `getSportsDbTeamId`: Implementálva a liga-alapú keresés elsődleges módszerként.
* - Keresi a liga ID-t (`/search/league/`).
* - Lekéri a csapatlistát a ligából (`/list/teams/`).
* - Ebben keresi a csapatot.
* - Fallback: `/search/team/` keresés, de a találatokat szűri a `strSport` alapján.
* - Liga ID gyorsítótárazás hozzáadva.
**************************************************************/
        ...
        try {
            ...
            return response;
             ...
             if (error.response) {
                 ...
                 // Ha API kulcs hiba vagy rate limit, ne próbálkozzon újra
                 ...
                 // Feldolgozhatatlan kérésnél se próbálkozzon újra
             ...
             else { // Axios beállítási hiba
                 errorMessage += `Beállítási hiba: ${error.message}`;
                 console.error(errorMessage, error.stack); return null; // Ne próbálkozzon újra
             }
             console.warn(errorMessage);
             if (attempts <= retries) { await new Promise(resolve => setTimeout(resolve, 1500 * attempts)); } // Várakozás
             else { console.error(`API (${method}) hívás végleg sikertelen: ${url.substring(0, 150)}...`); return null; }
        }
    }
    console.error(`API (${method}) hívás váratlanul véget ért: ${url.substring(0, 150)}...`); // Elvileg nem érhet ide
    return null;
}
...
async function findSportMonksTeamId(teamName) {
    ...
    const TEAM_NAME_MAP = { /* ... (marad a régi map) ... */ 'genk': 'KRC Genk', 'betis': 'Real Betis', 'red star': 'Red Star Belgrade', 'sparta': 'Sparta Prague', 'inter': 'Internazionale', 'fc copenhagen': 'Copenhagen', 'manchester utd': 'Manchester United', 'atletico': 'Atletico Madrid', 'as roma': 'Roma' };
    ...
    for (let attempt = 0; attempt < namesToTry.length; attempt++) {
        ...
        try {
            ...
            else if (response.status !== 404) { console.warn(`SportMonks API figyelmeztetés (${response.status}) Keresés: "${searchName}".`); break; } // Ne próbálkozzon tovább, ha nem 404
        } catch (error) { console.error(`Hiba a SportMonks csapat ID lekérésekor ("${searchName}"): ${error.message}`); if (!axios.isAxiosError(error) || error.code !== 'ECONNABORTED') break; } // Ne próbálkozzon tovább nem timeout hiba esetén
...
export async function _callGemini(prompt) {
    ...
    // A prompt végére kerülő JSON kényszerítő instrukciók maradnak
...
async function getSportsDbLeagueId(leagueName) {
    ...
    const cacheKey = `tsdb_leagueid_v1_${encodeURIComponent(leagueName.toLowerCase().replace(/\s+/g, ''))}`;
    ...
    const url = `https://www.thesportsdb.com/api/v2/json/search/league/${encodeURIComponent(leagueName)}`;
    console.log(`TheSportsDB V2 League Search próbálkozás: URL=${url.replace(THESPORTSDB_API_KEY,'<apikey>')}`); // API kulcsot nem logoljuk
    try {
        const response = await makeRequest(url, config);
        const leaguesArray = response?.data?.search || response?.data?.countrys; // 'countrys' a TSDB API furcsasága lehet
        if (leaguesArray && Array.isArray(leaguesArray) && leaguesArray.length > 0) {
            // Itt most az első találatot vesszük, feltételezve, hogy a név elég pontos
            const leagueId = leaguesArray[0].idLeague;
            console.log(`TheSportsDB: Liga ID találat "${leagueName}" -> "${leaguesArray[0].strLeague}" -> ${leagueId}`);
            sportsDbLeagueIdCache.set(cacheKey, leagueId);
            return leagueId;
        } else {
            console.warn(`TheSportsDB: Nem található liga ID ehhez: "${leagueName}". Válasz:`, JSON.stringify(response?.data).substring(0, 100));
            sportsDbLeagueIdCache.set(cacheKey, 'not_found');
            return null;
        }
    } catch (error) {
        console.error(`TheSportsDB Hiba a(z) "${leagueName}" liga keresésekor: ${error.message}`);
        sportsDbLeagueIdCache.set(cacheKey, 'not_found'); // Hibát is cache-elünk
        return null;
    }
}
...
async function getSportsDbTeamId(teamName, sport, leagueName) {
    ...
    // Cache kulcs most már tartalmazza a sportágat és a ligát is (ha van)
    ...
    const config = { headers: TSDB_HEADERS, method: 'GET' };
    let teamId = null;
    const sportNameMapping = { // TheSportsDB sportágnevek
        soccer: 'Soccer',
        hockey: 'Ice Hockey',
    ...
    if (leagueName && targetSportName) {
        ...
        if (leagueId) {
            ...
            try {
                ...
                const teamsInLeague = listResponse?.data?.list || listResponse?.data?.teams; // 'teams' lehet a V1 kompatibilitás miatt?
                if (teamsInLeague && Array.isArray(teamsInLeague) && teamsInLeague.length > 0) {
                    ...
                    const variations = generateTeamNameVariations(teamName); // Csapatnév variációk
                    let bestMatchResult = { bestMatch: { rating: 0 } };
                    let foundTeam = null;

                    // Először próbáljunk meg pontos vagy nagyon hasonló egyezést találni
                    for (const variation of variations) {
                        const exactMatch = teamsInLeague.find(t => t.strTeam.toLowerCase() === variation.toLowerCase());
                        if (exactMatch) {
                            foundTeam = exactMatch;
                            bestMatchResult = { bestMatch: { rating: 1.0 } }; // Pontos találat
                            break;
                        }
                    }

                    // Ha nincs pontos, használjuk a string-similarity-t
                    if (!foundTeam) {
                        const matchResult = findBestMatch(teamName, teamNamesFromLeague);
                        if (matchResult.bestMatch.rating > 0.65) { // Itt lehet játszani a küszöbbel
    ...
    if (!teamId && targetSportName) {
        ...
        const namesToTry = generateTeamNameVariations(teamName); // Csapatnév variációk generálása
        ...
        for (const searchName of namesToTry) {
            ...
            try {
                ...
                if (teamsArray && Array.isArray(teamsArray) && teamsArray.length > 0) {
                    ...
                    if (teamsInSport.length > 0) {
                        if (teamsInSport.length === 1) {
                            ...
                            // Ha több találat maradt a szűrés után is, használjuk a string-similarity-t
                            const teamNamesFromSport = teamsInSport.map(t => t.strTeam);
                            const bestApiMatch = findBestMatch(teamName, teamNamesFromSport);
                            if (bestApiMatch.bestMatch.rating > 0.65) { // Küszöb
                ...
                if (error.response && [401, 403, 429].includes(error.response.status)) { break; } // Ne próbálkozzon tovább authentikációs/limit hiba esetén
            }
            await new Promise(resolve => setTimeout(resolve, 50)); // Kis szünet a próbálkozások között
        }
...
async function getSportsDbMatchId(leagueName, homeTeamId, awayTeamId) {
    ...
    try {
        ...
        if (responseNext?.data?.events) {
            ...
            // Most már pontosabb ID alapján keresünk
    ...
    if (!matchId) { // Ha a következő meccsek közt nincs, nézzük az előzőeket (max 2 napra visszamenőleg)
        ...
        try {
            ...
            if (responsePrev?.data?.results) {
                ...
                const recentMatch = events.sort((a, b) => { /* Dátum rendezés marad */ const dateA = new Date(a.dateEvent + 'T' + (a.strTime || '00:00:00')); const dateB = new Date(b.dateEvent + 'T' + (b.strTime || '00:00:00')); if(isNaN(dateA.getTime())) return 1; if(isNaN(dateB.getTime())) return -1; return dateB - dateA; })
                                      .find(e => String(e.idHomeTeam) === String(homeTeamId) && String(e.idAwayTeam) === String(awayTeamId)); // Pontos ID keresés
                if (recentMatch) {
...
async function getSportsDbPlayerList(teamId) {
    ...
    try {
        ...
        const players = response?.data?.player || response?.data?.list; // 'list' kulcsot is nézzük
...
async function getSportsDbRecentMatches(teamId) {
    ...
    try {
        const response = await makeRequest(url, config);
        if (response === null) { sportsDbCache.set(cacheKey, 'not_found'); return null; }
        const matches = response?.data?.results || response?.data?.schedule; // 'schedule' kulcsot is nézzük
        if (Array.isArray(matches) && matches.length > 0) { console.log(`TheSportsDB (V2/Header): ${matches.length} legutóbbi meccs lekérve (${teamId}).`); const relevantMatches = matches .map(m => ({ idEvent: m.idEvent, strEvent: m.strEvent, dateEvent: m.dateEvent, strTime: m.strTime, intHomeScore: m.intHomeScore, intAwayScore: m.intAwayScore, strHomeTeam: m.strHomeTeam, strAwayTeam: m.strAwayTeam })).sort((a,b) => { /* Rendezés marad */ const dateA = new Date(a.dateEvent + 'T' + (a.strTime || '00:00:00')); const dateB = new Date(b.dateEvent + 'T' + (b.strTime || '00:00:00')); if (isNaN(dateA.getTime())) return 1; if (isNaN(dateB.getTime())) return -1; return dateB - dateA; }); sportsDbCache.set(cacheKey, relevantMatches); return relevantMatches; }
...
async function getSportsDbLineups(matchId) {
    ...
    try {
        const response = await makeRequest(url, config);
        if (response === null) { sportsDbCache.set(cacheKey, 'not_found'); return null; }
        const lineups = response?.data?.lineup || response?.data?.lookup; // 'lookup' kulcsot is nézzük
...
async function getSportsDbMatchStats(matchId) {
    ...
    try {
        const response = await makeRequest(url, config);
        if (response === null) { sportsDbCache.set(cacheKey, 'not_found'); return null; }
        const stats = response?.data?.eventstats || response?.data?.lookup; // 'lookup' kulcsot is nézzük
...
// --- FŐ ADATGYŰJTŐ FUNKCIÓ --- (Változatlan, de most már a javított getSportsDbTeamId-t hívja)
export async function getRichContextualData(sport, homeTeamName, awayTeamName, leagueName, utcKickoff) {
    ...
    // Cache kulcs verzió növelve (v40), hogy az új ID keresést használja
    const ck = `rich_context_v40_liga_fallback_${sport}_${encodeURIComponent(teamNames[0])}_${encodeURIComponent(teamNames[1])}`;
    const cached = scriptCache.get(ck);
    if (cached) {
        console.log(`Cache találat (${ck})`);
        // Odds adatokat itt is frissíthetjük, ha kell
        const oddsResult = await getOptimizedOddsData(homeTeamName, awayTeamName, sport, SPORT_CONFIG[sport], null, leagueName);
        if (oddsResult && !oddsResult.fromCache) { return { ...cached, fromCache: true, oddsData: oddsResult }; } // Ha az odds frissült
    ...
    try {
        ...
        // *** JAVÍTOTT getSportsDbTeamId HÍVÁS ***
        const [homeTeamId, awayTeamId] = await Promise.all([
            getSportsDbTeamId(homeTeamName, sport, leagueName), // Átadjuk a sportágat és a ligát
            getSportsDbTeamId(awayTeamName, sport, leagueName)  // Átadjuk a sportágat és a ligát
        ]);
        let matchId = null; let lineups = null; let matchStats = null;
        if (homeTeamId && awayTeamId) { // Csak ha mindkét (remélhetőleg helyes) ID megvan
        ...
        // Prompt összeállítása marad (de most már jobb TSDB adatokkal kellene rendelkeznie)
        ...
        const extractLineup = (lineupData, teamId) => { /* ... (marad) ... */ if (!lineupData || !Array.isArray(lineupData)) return 'N/A'; const teamLineup = lineupData.find(l => String(l.idTeam) === String(teamId) && l.strFormation); return teamLineup?.strLineup || 'N/A'; };
        ...
        const finalData = {};
        // Adatok feldolgozása marad
        const parseStat = (val, d = null) => { if (val === null || val === undefined || val === "N/A") return d; const n = Number(val); return (!isNaN(n) && n >= 0) ? n : d; };
        const inferGp = (f) => { if (!f || typeof f !== 'string' || f === "N/A") return 5; const m = f.match(/[WDL]/g); return m ? Math.min(m.length, 10) : 5; };
        let dGpHome = inferGp(geminiData?.form?.home_overall); let dGpAway = inferGp(geminiData?.form?.away_overall);
        let homeGp = parseStat(geminiData?.stats?.home?.gp, dGpHome); let awayGp = parseStat(geminiData?.stats?.away?.gp, dGpAway); homeGp = Math.max(1, homeGp || 1); awayGp = Math.max(1, awayGp || 1);
        ...
        // Kritikus statisztikák ellenőrzése marad
        if (typeof result.rawStats?.home !== 'object' || typeof result.rawStats?.away !== 'object' || typeof result.rawStats.home.gp !== 'number' || result.rawStats.home.gp <= 0 || typeof result.rawStats.away.gp !== 'number' || result.rawStats.away.gp <= 0) { console.error(`KRITIKUS HIBA (${homeTeamName} vs ${awayTeamName}): Érvénytelen statisztikák. HomeGP: ${result.rawStats?.home?.gp}, AwayGP: ${result.rawStats?.away?.gp}`); throw new Error(`Kritikus statisztikák érvénytelenek a ${homeTeamName} vs ${awayTeamName} meccshez.`); }
        scriptCache.set(ck, result); // Cache mentése
        console.log(`Sikeres adatgyűjtés (AI + TSDB(ID,Pl,M) + Időjárás), cache mentve (${ck}).`);
        return { ...result, fromCache: false, oddsData: fetchedOddsData }; // Visszatérés az eredménnyel és az oddsokkal
...
async function getOddsData(homeTeam, awayTeam, sport, sportConfig, leagueName) {
    ...
    try {
        ...
        for (const match of oddsData) { // Megfelelő meccs keresése marad
        ...
        // H2H és Totals adatok kinyerése marad
...
export async function getOptimizedOddsData(homeTeam, awayTeam, sport, sportConfig, openingOdds, leagueName = null) {
    ...
    if (!liveOdds && leagueName && getOddsApiKeyForLeague(leagueName) !== sportConfig.odds_api_sport_key) {
        ...
        liveOdds = await getOddsData(homeTeam, awayTeam, sport, sportConfig, null); // Alap kulccsal
    }
...
function generateTeamNameVariations(teamName) {
     ...
     // Csak 2 karakternél hosszabb, nem üres stringek
...
export function findMainTotalsLine(oddsData) {
    ...
    for (const point of points) { // Legközelebbi odds pár keresése marad
    ...
    if (closestPair.diff < 0.5) return closestPair.line; // Ha van szoros pár
    // Ha nincs, a default vonalhoz legközelebbi pontot adjuk vissza
...
export async function _getFixturesFromEspn(sport, days) {
    ...
    for (const dateString of datesToFetch) { // Dátumokon és ligákon iterálás marad
        for (const [leagueName, slug] of Object.entries(sportConfig.espn_leagues)) {
            ...
            promises.push( makeRequest(url, { timeout: 8000 }).then(response => { // API hívás
                if (!response?.data?.events) return [];
                // Események feldolgozása és szűrése marad
            ...
            }).catch(error => { /* Hibakezelés marad */ if (error.response?.status === 400 || error.message.includes('404')) console.warn(`ESPN Hiba (40x): Lehetséges, hogy rossz a slug '${slug}' (${leagueName})? URL: ${url}`); else console.error(`ESPN Hiba (${leagueName}, ${slug}): ${error.message}`); return []; }));
            await new Promise(resolve => setTimeout(resolve, 50)); // Kis késleltetés marad
        }
    }
    try { // Eredmények összesítése és rendezése marad
