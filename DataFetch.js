        ...
        try {
            ...
            if (response.status < 200 || response.status >= 300) {
                 ...
                 if ([401, 403].includes(response.status)) {
                    console.error(`TheSportsDB Hitelesítési Hiba (${response.status})! Ellenőrizd az API kulcsot a .env fájlban és a Render környezeti változókban! Küldött header:`, currentConfig.headers);
                 }
            ...
            return response;
             ...
             if (error.response) {
                 ...
                 // Header hiba ("Missing API key") vagy rate limit (429) esetén nincs újrapróbálkozás
...
export async function _callGemini(prompt) {
    ...
    try {
        ...
        if (firstBrace === -1 || lastBrace === -1 || lastBrace < firstBrace) {
             console.error("A kapott válasz nem tűnik JSON objektumnak:", potentialJson.substring(0, 500));
        ...
        throw e; // Továbbdobjuk a hibát, hogy a hívó (getAndParse) elkapja
    }
}
...
/**
 * ÚJ FUNKCIÓ: Lekéri a TheSportsDB Match ID-t a csapat ID-k alapján.
 */
async function getSportsDbMatchId(homeTeamId, awayTeamId) {
    if (!THESPORTSDB_API_KEY || !homeTeamId || !awayTeamId) return null;

    const cacheKey = `tsdb_matchid_${homeTeamId}_${awayTeamId}`;
    const cachedId = sportsDbCache.get(cacheKey);
    if (cachedId) return cachedId === 'not_found' ? null : cachedId;

    const url = `https://www.thesportsdb.com/api/v2/json/schedule/next/team/${homeTeamId}`; // A hazai csapat következő meccseit nézzük
    const config = { headers: TSDB_HEADERS };
    ...
    try {
        ...
        const match = events.find(e => e.idAwayTeam === awayTeamId); // Megkeressük a meccset az ellenfél ID-ja alapján
        const matchId = match?.idEvent || null;

        if (matchId) {
            console.log(`TheSportsDB: Match ID ${matchId} találat.`);
            sportsDbCache.set(cacheKey, matchId);
            return matchId;
        } else {
            console.warn(`TheSportsDB: Nem található Match ID ehhez: ${homeTeamId} vs ${awayTeamId}.`);
            sportsDbCache.set(cacheKey, 'not_found');
            return null;
        }
    } catch (error) {
        console.error(`TheSportsDB Hiba (getSportsDbMatchId): ${error.message}`);
        sportsDbCache.set(cacheKey, 'not_found');
        return null;
    }
}
...
async function getSportsDbLineups(matchId) {
    ...
    try {
        ...
        if (Array.isArray(lineups)) {
            console.log(`TheSportsDB (V2): Kezdőcsapatok lekérve a ${matchId} meccshez.`);
...
async function getStructuredWeatherData(stadiumLocation, utcKickoff) {
    ...
    if (latLonMatch && latLonMatch[1] && latLonMatch[2]) {
        ...
        try {
            ...
            if (!geoResponse?.data?.results?.[0]) {
                console.warn(`Időjárás API: Geokódolás sikertelen: ${stadiumLocation}`);
                return null;
            }
    ...
    try {
        ...
        if (!hourlyData?.time || !hourlyData?.precipitation || !hourlyData?.wind_speed_10m) {
            console.warn(`Időjárás API: Hiányos válasz (${stadiumLocation}).`, weatherResponse?.data);
            return null;
        }
        ...
        for (let i = 0; i < hourlyData.time.length; i++) {
             ...
             if (apiTime.startsWith(targetTimeISO.substring(0, 13))) {
                 timeIndex = i;
                 break;
             }
        }
        ...
        if (timeIndex === -1) {
            ...
            console.warn("Időjárás API: Nincs órás adat.");
            return null;
        }
        ...
        console.error(`Időjárás API hiba: ${e.message}`);
        return null;
    }
}
...
export async function getRichContextualData(sport, homeTeamName, awayTeamName, leagueName, utcKickoff) {
    ...
    try {
        ...
        const matchId = await getSportsDbMatchId(homeTeamId, awayTeamId);
        ...
        const [homePlayers, awayPlayers, homeMatches, awayMatches, lineups, matchStats] = await Promise.all([
            ...
            matchId ? getSportsDbLineups(matchId) : Promise.resolve(null),
            matchId ? getSportsDbMatchStats(matchId) : Promise.resolve(null),
        ]);
        ...
        console.log(`TheSportsDB adatok lekérve: H_ID=${homeTeamId || 'N/A'}, A_ID=${awayTeamId || 'N/A'}, MatchID=${matchId || 'N/A'}, Lineups: ${lineups ? 'Van' : 'Nincs'}`);

        // --- 2. LÉPÉS: Gemini AI Hívás (PROMPT_V38) ---
        ...
        const PROMPT_V38 = `CRITICAL TASK: Analyze the ${sport} match: "${homeTeamName}" (Home) vs "${awayTeamName}" (Away). Provide a single, valid JSON object. Focus ONLY on the requested fields. **CRITICAL: You MUST use the latest factual data provided below over your general knowledge.**
...
- Official Lineups (if available): ${JSON.stringify(sportsDbData.lineups || 'N/A').substring(0, 500)}

REQUESTED ANALYSIS (Fill in based on your knowledge AND the provided factual data):
1. Basic Stats: gp, gf, ga. If not available, use "N/A" for number fields.
2. H2H: Last 5 structured + summary.
3. Team News & Absentees: Key absentees (name, importance, role) + news summary + impact. (CRITICAL: Use Lineups/Player List to verify status).
4. Recent Form: W-D-L strings based on provided match history.
5. Key Players: name, role, recent key stat.
6. Contextual Factors: Stadium Location, Match Tension, Pitch Condition, Referee.
--- SPECIFIC DATA BY SPORT ---
IF soccer:
7. Tactics: Style + formation. (Use Lineup data to infer formation if possible).
8. Tactical Patterns: { home: [], away: [] }.
9. Key Matchups: { }.
OUTPUT FORMAT: Strict JSON as defined in previous versions. Use "N/A" or null appropriately.`;
        ...
        try {
            ...
            console.error(`Gemini JSON parse hiba: ${e.message}`);
        }
        ...
        const finalData = {};
        const parseStat = (val, d = null) => {
            if (typeof val === 'number' && !isNaN(val) && val >= 0) return val;
            if (val === "N/A" || val === null || val === undefined) return d;
            const num = parseInt(val, 10);
            return (typeof num === 'number' && !isNaN(num) && num >= 0) ? num : d;
        };
