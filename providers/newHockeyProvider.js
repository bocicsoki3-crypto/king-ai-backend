...
// Ennek az API-nak nincs kulcsrotációja, egyetlen kulcsot használunk
async function makeHockeyRequest(endpoint, params = {}) {
    const { HOCKEY_API_KEY, HOCKEY_API_HOST } = process.env;

    if (!HOCKEY_API_KEY || !HOCKEY_API_HOST) {
        throw new Error('[Hockey API] Hiányzó HOCKEY_API_KEY vagy HOCKEY_API_HOST a .env fájlban.');
    }
    ...
    try {
        ...
        console.error(`[Hockey API Hiba] Endpoint: ${endpoint} - ${error.message}`);
        if (error.response?.status === 401 || error.response?.status === 403) {
            console.error("[Hockey API] HITELESÍTÉSI HIBA. Ellenőrizd a HOCKEY_API_KEY-t és az előfizetést.");
        }
        return null; // Hibás válasz esetén null-t adunk vissza
    }
}
...
export async function fetchMatchData(options) {
    ...
    // Az 'apiSportsProvider'-hoz hasonló struktúrát hozunk létre
    const unifiedHomeStats = {
        gamesPlayed: homeStats?.played || 0,
        form: homeStats?.form || 'N/A', // Ez az API 'form' stringet ad (pl. "LWWWL")
    ...
    // A 'PROMPT_V43' ugyanúgy használható, mint a focinál
    ...
    // Ugyanazt az egységes 'result' objektumot hozzuk létre,
    // mint amit az 'apiSportsProvider' is tenne.
    ...
    // A 'richContext' felépítése a Gemini és a statisztikák alapján
    const richContext = [
