...
import {
    ...
    API_HOSTS
    // Figyelem: Az XG_API kulcsokat most már közvetlenül a 'process.env'-ből olvassuk
...
// Ez a függvény most már közvetlenül 'process.env'-ből olvassa a kulcsokat
...
async function getXgData(fixtureId) {
    ...
    // KÖZVETLEN beolvasás a 'process.env'-ből.
    // Győződj meg róla, hogy a .env fájl be van töltve az alkalmazás indításakor!
    const apiKey = process.env.XG_API_KEY;
    const apiHost = process.env.XG_API_HOST;

    if (!apiKey || !apiHost) {
        console.warn("xG API: Hiányzó XG_API_KEY vagy XG_API_HOST a környezeti változókban (.env). xG lekérés kihagyva.");
        return null;
    }
    ...
    const options = {
        ...
        headers: {
            'X-RapidAPI-Key': apiKey,  // Garantáltan a process.env-ből
            'X-RapidAPI-Host': apiHost // Garantáltan a process.env-ből
        },
