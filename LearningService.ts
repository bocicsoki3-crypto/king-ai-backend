// LearningService.ts (v94.0 - "Önjavító Hurok")
// MÓDOSÍTÁS (v94.0):
// 1. MÓDOSÍTVA: A 'PROMPT_AUDITOR_V1' (v71.2) frissítve 'PROMPT_AUDITOR_V94'-re.
// 2. LOGIKA: Az új prompt már nem csak szöveges elemzést, hanem egy strukturált
//    'learnings: []' tömböt is generál, amely kompatibilis az 'updateNarrativeRatings'
//    funkcióval (pl. [{ team: "Atalanta", rating: "pressure_handling", adjustment: -1 }]).
// 3. MÓDOSÍTVA: A 'runAuditAnalysis' funkció most már meghívja az
//    'updateNarrativeRatings(auditResult.learnings)'-t.
// 4. CÉL: A 7. Ügynök (Revizor) által talált hiba automatikusan visszacsatolásra
//    kerül a "Narratív Cache"-be, amit az AnalysisFlow v94.0 már beolvas.
...
export function updateNarrativeRatings(learnings: ILearning[]): void {
    ...
    ratingCache.set(NARRATIVE_RATINGS_KEY, narrativeRatings); 
...
// === MÓDOSÍTVA (v94.0): 7. ÜGYNÖK (AZ AUDITOR) PROMPT ===
// Ez a prompt most már strukturált 'learnings' tömböt ad ki.
...
// === MÓDOSÍTVA (v94.0): 7. ÜGYNÖK (AUDITOR) FUNKCIÓ ===
/**
...
export async function runAuditAnalysis(
    ...
    try {
        ...
        if (auditResult && auditResult.flaw_analysis && auditResult.corrective_insight && auditResult.learnings) {
            ...
            await logLearningInsight(process.env.SHEET_URL || '', {
                ...
                confidence: confidence,
                actual: actualResult,
                insight: `[AUDITOR v94.0] Hiba: ${auditResult.flaw_analysis} | Javaslat: ${auditResult.corrective_insight} | Tanulságok: ${JSON.stringify(auditResult.learnings)}`
            });
