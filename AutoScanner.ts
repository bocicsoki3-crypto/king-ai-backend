...
export async function runSniperScan(sportType: 'soccer' | 'us_sports') {
    ...
    try {
        ...
        if (results.length > 0) {
            ...
            console.log(`[AutoScanner] Nem találtam 7% feletti value-t a(z) ${sportType} kategóriában.`);
            // Küldünk egy értesítést akkor is, ha nincs semmi? Inkább ne zavarjuk a felhasználót.
        }
...
async function sendEmailReport(type: string, results: any[]) {
    const subject = `🔥 King AI Sniper Report - ${type === 'soccer' ? 'Foci' : 'Kosár/Hoki'} (${new Date().toLocaleDateString('hu-HU')})`;
    
    let html = `
        <h2 style="color: #d32f2f;">King AI Sniper - v147.0 Victory Protocol</h2>
        <p>A rendszer az alábbi meccseket találta, ahol a <b>matematikai előny meghaladja a 7%-ot</b>.</p>
        <hr>
    `;
