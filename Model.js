...
// Max 95% esély eseményre
    ...
    const probabilities = [
        ...
        { type: 'AWAY_GOAL', probability: awayGoalProb_SetPiece 
* scaleFactor, team: 'away', detail: 'Pontrúgás' },
...
logData.source += ' (Korlátozva)';
            ...
            logData.base_mu_h_sm = mu_h;
// Külön logoljuk, ha SM-ből jön
...
mu_h *= home_adv_mod; mu_a *= away_adv_mod;
    ...
    // --- MÓDOSÍTÁS KEZDETE (2025.10.25) - H2H DOMINANCIA SÚLYOZÁS ---
    if (sport === 'soccer' || sport === 'hockey' || sport === 'basketball') {
        logData.step = 'H2H Dominancia Súlyozás';
        const H2H_WEIGHTING_FACTOR = 0.05; 
        let h2hHomeAdjust = 1.0;
        let h2hAwayAdjust = 1.0;
        const h2hStructured = rawData?.h2h_structured || [];
...
export function calculatePsychologicalProfile(teamName, opponentName, rawData = null) {
    ...
    if (rawData) {
        ...
        // Meccs tétjének hatása a nyomásra (JAVÍTVA: if/else if)
        const tension = rawData.contextual_factors?.match_tension_index?.toLowerCase();
        
        if (tension === 'extreme') {
             pressureIndex *= 1.10; // Még nagyobb nyomás
        } else if (tension === 'high') {
             pressureIndex *= 1.05; // Nagyobb nyomás
