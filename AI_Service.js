 ...
 * VÉGLEGES JAVÍTÁS: Minden prompt a JSON belső struktúráját is kikényszeríti. A Mester AI bátrabb.
 */
import { getRichContextualData, getOptimizedOddsData, _callGemini, _getFixturesFromEspn } from './DataFetch.js';
import { calculateProbabilities, generateProTip, simulateMatchProgress, estimateXG, estimateAdvancedMetrics, calculateModelConfidence, calculatePsychologicalProfile, calculateValue, analyzeLineMovement, analyzePlayerDuels, buildPropheticTimeline } from './Model.js';
...
const EXPERT_CONFIDENCE_PROMPT = `You are a master betting risk analyst. Provide a confidence score and justification in Hungarian. Start with Statistical Model Confidence ({modelConfidence}/10) and adjust it based on the Narrative Context ({richContext}).
...
const AI_KEY_QUESTIONS_PROMPT = `You are a strategic analyst. Based on the context, formulate the two most critical strategic questions in Hungarian that will decide the match. Present as a bulleted list. CONTEXT: {richContext}.
...
const PLAYER_MARKETS_PROMPT = `You are a player performance markets specialist. Suggest 1-2 player-specific betting markets in Hungarian (2-3 sentences max). Highlight player names & markets with **asterisks**. DATA: Key Players: {keyPlayersJson}, Context: {richContext}.
...
const BTTS_ANALYSIS_PROMPT = `You are a BTTS specialist. Analyze if both teams will score. DATA: Sim BTTS: {sim_pBTTS}%, xG: H {sim_mu_h} - A {sim_mu_a}. Conclude with confidence.
...
const SOCCER_GOALS_OU_PROMPT = `You are a Soccer O/U specialist. Analyze total goals vs line ({line}). DATA: Sim Over: {sim_pOver}%, xG Sum: {sim_mu_sum}. Conclude with confidence.
...
const STRATEGIC_CLOSING_PROMPT = `You are the Master Analyst. Craft "Stratégiai Zárógondolatok" (2-3 Hungarian paragraphs). Discuss promising angles AND risks from all data. DATA: Scenario: "{propheticScenario}", Stats: H:{sim_pHome}%, O/U: O:{sim_pOver}%, Market: "{marketIntel}", Micromodels: {microSummaryJson}, Context: {richContext}, Risk Assessment: "{riskAssessment}".
...
// === AI elemzők, amelyek a JSON-ból a megfelelő kulcsot veszik ki ===
...
export async function getTacticalBriefing(rawData, sport, home, away, duelAnalysis) { return await getAndParse(TACTICAL_BRIEFING_PROMPT, { sport, home, away, home_style: rawData.tactics?.home?.style, away_style: rawData.tactics?.away?.style, duelAnalysis, key_players_home: rawData.key_players?.home?.map(p => p.name).join(', '), key_players_away: rawData.key_players?.away?.map(p => p.name).join(', ') }, "analysis"); }

export async function getPropheticScenario(propheticTimeline, rawData, home, away, sport) { return await getAndParse(PROPHETIC_SCENARIO_PROMPT, { sport, home, away, timelineJson: JSON.stringify(propheticTimeline || []), home_style: rawData.tactics?.home?.style, away_style: rawData.tactics?.away?.style, tension: rawData.contextual_factors?.match_tension_index }, "scenario"); }
...
export async function getRiskAssessment(sim, rawData, sport, marketIntel) { return await getAndParse(RISK_ASSESSMENT_PROMPT, { sport, sim, marketIntel, news_home: rawData.team_news?.home, news_away: rawData.team_news?.away, form_home: rawData.form?.home_overall, form_away: rawData.form?.away_overall }, "risk_analysis"); }

export async function getFinalGeneralAnalysis(sim, tacticalBriefing, propheticScenario) { return await getAndParse(FINAL_GENERAL_ANALYSIS_PROMPT, { sim, mu_h: sim.mu_h_sim, mu_a: sim.mu_a_sim, tacticalBriefing, propheticScenario }, "general_analysis"); }
...
export async function getBTTSAnalysis(sim, rawData) { return await getAndParse(BTTS_ANALYSIS_PROMPT, { sim, ...rawData.form }, "btts_analysis"); }

export async function getSoccerGoalsOUAnalysis(sim, rawData, mainTotalsLine) { return await getAndParse(SOCCER_GOALS_OU_PROMPT, { line: mainTotalsLine, sim, ...rawData.form, ...rawData.tactics }, "goals_ou_analysis"); }
...
export async function getStrategicClosingThoughts(sim, rawData, richContext, marketIntel, microAnalyses, riskAssessment) { const microSummary = Object.entries(microAnalyses || {}).map(([key, val]) => `${key}: ${val}`).join('; '); return await getAndParse(STRATEGIC_CLOSING_PROMPT, { sim, propheticScenario: rawData.propheticScenario, marketIntel, microSummaryJson: microSummary, richContext, riskAssessment }, "strategic_analysis"); }
...
export async function getMasterRecommendation(valueBets, sim, modelConfidence, expertConfidence, riskAssessment, microAnalyses, generalAnalysis, strategicClosingThoughts) {
    ...
    const data = { valueBetsJson: JSON.stringify(valueBets || []), sim, modelConfidence, expertConfidence, riskAssessment, microSummary, generalAnalysis, strategicClosingThoughts };
    try {
    ...
    } catch (e) { return { "recommended_bet": "Nincs fogadás", "final_confidence": 1.0, "brief_reasoning": `AI Hiba: ${e.message}` }; }
}
