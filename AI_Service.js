    ...
    return template.replace(/\{([\w_]+)\}/g, (match, key) => { //
        ...
        try {
            ...
            if (typeof value === 'number' && !isNaN(value)) { //
                if (key.startsWith('sim_p') || key.startsWith('modelConfidence') ||
                    key.endsWith('_svp') || key.endsWith('_pct')) return value.toFixed(1); //
...
// === MÓDOSÍTÁS: AI elemzők, automatikus újrapróbálkozással ===

/**
 * Robust AI hívó segédfüggvény.
 * Meghívja az AI-t a prompttal, és automatikusan újrapróbálkozik hiba esetén.
 * @param {string} prompt A prompt sablon.
 * @param {object} data Az adatok a prompt kitöltéséhez.
 * @param {string} key A JSON válaszból kinyerni kívánt kulcs.
 * @param {number} retries Az újrapróbálkozások száma (alapértelmezetten 1).
    ...
    while (attempts <= retries) {
        try {
            ...
            if (result && typeof result === 'object' && key in result) {
                return result[key]; // Siker!
            }

            // Ha a kulcs hiányzik, az strukturális hiba, nem próbáljuk újra.
            ...
            // Ez a 'catch' ág az _callGemini hibáit kapja el (pl. timeout, 500-as API hiba, rate limit)
            attempts++;
            console.warn(`AI Hiba a(z) ${key} feldolgozásakor (Próba: ${attempts}/${retries + 1}): ${e.message}`);
            
            if (attempts > retries) {
                // Elfogyott az újrapróbálkozás
                console.error(`Végleges AI Hiba (${key}) ${attempts} próbálkozás után.`);
                return `AI Hiba (${key}): ${e.message}`;
            }
            
            // Várakozás az újrapróbálkozás előtt (növekvő várakozás)
...
export async function getTacticalBriefing(rawData, sport, home, away, duelAnalysis, riskAssessment) { //
    const data = { //
        ...
        duelAnalysis: duelAnalysis ||
            'Nincs kiemelt párharc elemzés.', //
        key_players_home: rawData.key_players?.home?.map(p => p.name).join(', ') ||
            'N/A', //
        key_players_away: rawData.key_players?.away?.map(p => p.name).join(', ') ||
            'N/A' //
    };
...
export async function getFinalGeneralAnalysis(sim, mu_h, mu_a, tacticalBriefing, propheticScenario, rawData) { 
    return await getAndParse(FINAL_GENERAL_ANALYSIS_PROMPT, { 
        sim_pHome: sim.pHome, 
        sim_pDraw: sim.pDraw, 
        sim_pAway: sim.pAway, 
        mu_h: mu_h, 
...
export async function getBTTSAnalysis(sim, rawData) { 
    return await getAndParse(BTTS_ANALYSIS_PROMPT, { 
        sim_pBTTS: sim.pBTTS,
...
export async function getSoccerGoalsOUAnalysis(sim, rawData, mainTotalsLine) { 
    return await getAndParse(SOCCER_GOALS_OU_PROMPT, { 
        line: mainTotalsLine, 
        sim_pOver: sim.pOver,
        sim_mu_sum: (sim.mu_h_sim + sim.mu_a_sim)
    }, "goals_ou_analysis");
} //

export async function getCornerAnalysis(sim, rawData) { const likelyLine = 9.5;
    return await getAndParse(CORNER_ANALYSIS_PROMPT, { 
        likelyLine, 
        sim, 
        ...rawData.advanced_stats, 
        ...rawData.tactics 
    }, "corner_analysis");
} //

export async function getCardAnalysis(sim, rawData) { const likelyLine = 4.5;
    return await getAndParse(CARD_ANALYSIS_PROMPT, { 
        likelyLine, 
        sim, 
        ...rawData.advanced_stats, 
        ...rawData.referee, 
        ...rawData.contextual_factors 
    }, "card_analysis");
} //

export async function getHockeyGoalsOUAnalysis(sim, rawData, mainTotalsLine) { 
    return await getAndParse(HOCKEY_GOALS_OU_PROMPT, { 
        line: mainTotalsLine, 
        sim, 
        ...rawData.advanced_stats 
    }, "hockey_goals_ou_analysis");
} //

export async function getHockeyWinnerAnalysis(sim, rawData) { 
    return await getAndParse(HOCKEY_WINNER_PROMPT, { 
        sim, 
        ...rawData.advanced_stats, 
        ...rawData.form 
    }, "hockey_winner_analysis");
} //

export async function getBasketballPointsOUAnalysis(sim, rawData, mainTotalsLine) { 
    return await getAndParse(BASKETBALL_POINTS_OU_PROMPT, { 
        line: mainTotalsLine, 
        sim, 
        ff_home_json: JSON.stringify(rawData.advanced_stats?.home?.four_factors || {}), 
        ff_away_json: JSON.stringify(rawData.advanced_stats?.away?.four_factors || {}), 
        ...rawData.advanced_stats, 
        ...rawData.team_news 
...
export async function getMasterRecommendation(valueBets, sim, modelConfidence, expertConfidence, riskAssessment, microAnalyses, generalAnalysis, strategicClosingThoughts, rawData, contradictionAnalysisResult) { //
    ...
    const data = { //
        ...
        contradictionAnalysis: contradictionAnalysisResult // Átadjuk a placeholder/valós adatot
    };
    try {
        // A Mester Ajánlásnál több (pl. 2) újrapróbálkozást is engedhetünk, mert ez a legkritikusabb hívás
...
export async function getChatResponse(context, history, question) {
    ...
    try {
        ...
        //
        // Simplified prompt for chat, doesn't need strict JSON
...
If the answer isn't there, say so politely.`; //
        // Note: Using _callGemini for chat still forces JSON, which isn't ideal.
        // A separate _callGeminiForChat function without JSON enforcement would be better in the future.
        const jsonAnswer = await _callGemini(prompt);
        //
        // Attempt to extract text if it came back wrapped in JSON
        let answerText = jsonAnswer;
        try {
            const parsed = JSON.parse(jsonAnswer);
            // Look for a common key like 'answer' or the first string value
            answerText = parsed.answer ||
                parsed.response || Object.values(parsed).find(v => typeof v === 'string') || jsonAnswer;
        } catch (e) {
            // If parsing fails, assume it's plain text (or malformed JSON)
...
// --- FŐ EXPORT ---
export default {
    // runAnalysisFlow removed, belongs in AnalysisFlow.js
    getChatResponse, //
