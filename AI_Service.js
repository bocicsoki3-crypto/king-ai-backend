 ...
 * VÉGLEGES JAVÍTÁS: Minden prompt a JSON belső struktúráját is kikényszeríti. A Mester AI bátrabb. A kód teljes és vágatlan.
 */
...
export async function getRiskAssessment(sim, rawData, sport, marketIntel) { //
    const data = { //
        sport, sim, marketIntel: marketIntel || "N/A", //
...
export async function getStrategicClosingThoughts(sim, rawData, richContext, marketIntel, microAnalyses, riskAssessment) { //
    ...
    return await getAndParse(STRATEGIC_CLOSING_PROMPT, { sim, propheticScenario: rawData.propheticScenario, marketIntel, microSummaryJson: microSummary, richContext, riskAssessment }, "strategic_analysis"); //
}
