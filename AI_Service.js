...
**CRITICAL CONSTRAINT: You MUST NOT select any Corners or Cards markets for the final recommended_bet. Confine your selection to 1X2 (Hazai győzelem, Döntetlen, Vendég győzelem), Totals (Over/Under Goals/Points), or BTTS (Gólt szerez mindkét csapat - Igen/Nem) markets.**
...
CONTEXT: First, read the Tactical Briefing: "{tacticalBriefing}" **AND the Risk Assessment: "{riskAssessment}"**. Your narrative MUST match this tactical and risk assessment.
**CRITICAL RULE: If the Risk Assessment indicates a High or Very High level of risk (due to key player absences, poor form, or other factors), your predicted result MUST NOT be an extreme blowout (e.g., 5-0, 6-1, or large margins in Basketball/Hockey), but should reflect a tighter, more realistic scoreline (e.g., 2-1, 3-2, or a narrow point/goal margin).**
...
try {
  ...
  if (key.endsWith('Json')) {
            ...
            if 
(value === null || value === undefined) { return "N/A"; }
            if (typeof value === 'number' && !isNaN(value)) {
                if (key.startsWith('sim_p') || key.endsWith('_pct') || key === 'modelConfidence' ||
key === 'expertConfScore') return value.toFixed(1);
...
"N/A",
        home_formation: rawData?.tactics?.home?.formation ||
"N/A",
        away_formation: rawData?.tactics?.away?.formation ||
"N/A",
...
export async function getPropheticScenario(propheticTimeline, rawData, home, away, sport, tacticalBriefing, propheticEvent, riskAssessment) {
     const data = {
         sport, home, away,
         tacticalBriefing: tacticalBriefing ||
"N/A",
         propheticEvent: propheticEvent ||
"N/A",
         riskAssessment: riskAssessment ||
"N/A",
...
"N/A",
         key_players_home_names: rawData?.key_players?.home?.map(p => p.name).join(', ') ||
'N/A',
         key_players_away_names: rawData?.key_players?.away?.map(p => p.name).join(', ') ||
'N/A'
     };
...
export async function getExpertConfidence(modelConfidence, richContext, rawData = null) {
     const safeModelConfidence = typeof modelConfidence === 'number' ?
modelConfidence : 5.0;
     const data = {
...
"N/A",
        news_home: rawData?.team_news?.home ||
"N/A",
        news_away: rawData?.team_news?.away ||
"N/A",
        absentees_home_count: Array.isArray(rawData?.absentees?.home) ?
rawData.absentees.home.filter(p => p.importance === 'key').length : 0,
        absentees_away_count: Array.isArray(rawData?.absentees?.away) ?
rawData.absentees.away.filter(p => p.importance === 'key').length : 0,
        form_home: rawData?.form?.home_overall ||
"N/A",
        form_away: rawData?.form?.away_overall ||
"N/A",
...
export async function getBTTSAnalysis(sim, rawData) {
     const data = {
        ...
        home_style: rawData?.tactics?.home?.style ||
"N/A", away_style: rawData?.tactics?.away?.style || "N/A"
     };
...
"N/A", away_style: rawData?.tactics?.away?.style || "N/A"
    ...
    const likelyLine = sim?.mu_corners_sim ?
(Math.round(sim.mu_corners_sim - 0.1)) + 0.5 : 9.5;
...
export async function getCardAnalysis(sim, rawData) {
    const data = {
        ...
        referee_style: rawData?.referee?.style ||
"N/A",
        tension: rawData?.contextual_factors?.match_tension_index ||
"N/A",
...
export async function getHockeyGoalsOUAnalysis(sim, rawData, mainTotalsLine) {
     const data = {
        line: mainTotalsLine, sim_pOver: sim?.pOver, sim_mu_sum: (sim?.mu_h_sim ?? 0) + (sim?.mu_a_sim ?? 0),
        home_gsax: rawData?.advanced_stats_goalie?.home_goalie?.GSAx?.toFixed(2) ||
"N/A",
...
export async function getHockeyWinnerAnalysis(sim, rawData) {
     const data = {
        ...
        home_gsax: rawData?.advanced_stats_goalie?.home_goalie?.GSAx?.toFixed(2) ||
"N/A",
        away_gsax: rawData?.advanced_stats_goalie?.away_goalie?.GSAx?.toFixed(2) ||
"N/A",
        form_home: rawData?.form?.home_overall || "N/A", form_away: rawData?.form?.away_overall ||
"N/A"
     };
...
export async function getBasketballPointsOUAnalysis(sim, rawData, mainTotalsLine) {
     const data = {
        ...
        home_style: rawData?.shot_distribution?.home ||
"N/A",
...
const valueBetsString = JSON.stringify(valueBets || []);
    const data = {
        ...
        propheticScenario: propheticScenario ||
"N/A", tacticalBriefing: tacticalBriefing || "N/A",
        marketIntel: marketIntel ||
"N/A", microSummaryJson: microSummary,
        richContext: richContext || "Nincs kontextus.", riskAssessment: riskAssessment ||
"N/A",
        valueBetsJson: valueBetsString, modelConfidence: modelConfidence, expertConfidence: expertConfidence ||
"N/A"
     };
...
const data = {
        ...
        sim_mainTotalsLine: sim?.mainTotalsLine, sim_pOver: sim?.pOver,
        modelConfidence: safeModelConfidence,
        expertConfidence: expertConfidence ||
"N/A",
